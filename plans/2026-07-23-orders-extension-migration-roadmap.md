...
- **GĐ1 + GĐ2: ĐẠT — VERIFIED END-TO-END 2026-07-24 (chạy thật):** login Playwright (code cũ) → `/portal/shop` → đóng → **trình duyệt SẠCH + extension** đọc 3 shop → **tự bấm "Chi tiết" (trusted click) vào Seller Centre shop → KHÔNG captcha** → đọc "Chờ Lấy Hàng". Kiến trúc HYBRID: WebSocket (cổng cố định 47821) cho lệnh/dữ liệu + `chrome.debugger` cho trusted click, KHÔNG remote-debugging-port; đăng nhập do Playwright (subaccount + /portal/shop không bị captcha). Plan: `2026-07-23-gd1-cau-noi-extension-orders.md` + `2026-07-23-gd2-port-dang-nhap-extension.md` (pivot).
  - **Các "tường" đã vượt (ghi để GĐ3+ không vấp lại):** (1) **wsPort qua hash `#_od_ws` RỤNG** khi Shopee redirect → dùng CỔNG CỐ ĐỊNH 47821 + content.js gửi `wake`. (2) **Chrome/Brave single-instance handoff + orphan** cùng nối cổng cố định → cướp lệnh → `KillBrowsersOnProfile` (diệt mọi browser nạp `shopee-orders` trước clean launch). (3) **Service worker MV3 bị CACHE theo extension ID (=hash đường dẫn)** trong hồ sơ persistent → chạy SW cũ dù file đổi (reload ext tay mới ăn) → `PrepareFreshExtensionCopy` chép extension ra thư mục GUID mỗi lần → ID mới → SW mới. (4) **readShopList đọc trước khi bảng render → 0 shop** → poll 15s chờ `tr[data-row-key]`.
  - **Cần dọn trước/khi GĐ3:** badge chẩn đoán + `lastTabUrls` trong content.js/background.js (tạm); dead-code forwarder `ShopeeLoginService.OpenMailboxSignedInAsync` (ngoài); đa-lane: hạ tầng per-session copy đã có → chỉ cần BAKE cổng riêng vào mỗi copy (thay cổng cố định) để chạy song song nhiều acc.
- **GĐ3 (đọc/xử đơn 1 shop): chưa bắt đầu.** Port: đọc "Chờ Lấy Hàng" (đã có), vào "Tất cả", "Chuẩn bị hàng" + đặt địa chỉ lấy hàng, đọc bảng đơn + phân trang, tải/in phiếu — mỗi thao tác = 1 lệnh extension (executeScript + trusted click/type) + parser về C#. Tái dùng hàm thuần (ShopeeShippingNav/ScanOrdersJs...).
