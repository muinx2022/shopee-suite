...
| **2a** | Project chung `shared/Shopee.Proxy.Kiot` (HTTP client + parser kiotproxy.com duy nhất, inject được, không bao giờ ném) + orders chuyển sang adapter. KHÔNG hợp nhất pool — khảo sát 21/7 kết luận 3 bộ pool (ProxyPool IP-mới/nextRequestAt, KiotProxyPoolStore ánh-xạ-vị-trí, orders key-theo-phiên/sticky/watchdog) trái ngược bản chất, giữ riêng | Orders dùng client chung, 720 test cũ pass không sửa test |
| **2b** | Suite chuyển 2 dạng gọi (KiotResult + Dictionary/MultiBrave) thành adapter trên client chung — PHẢI giữ hợp đồng message `IsProxyExpiredError` (`BraveInstanceSession.cs:1297`); suite không có test | Nghiệm thu runtime thật: CheckAccount + Search + scrape MultiBrave chạy như cũ |
...
- 2026-07-21: **phase 2a hoàn thành** (`23fad0b`) — `shared/Shopee.Proxy.Kiot` + orders adapter, 732 test. Phase 2b (suite → client chung) CHƯA làm, cần nghiệm thu runtime scrape/check của người dùng.
- 2026-07-21: merge việc ngoài lộ trình "hồ sơ trình duyệt riêng theo từng browser" (`2e0d755`, plan `2026-07-21-ho-so-rieng-theo-trinh-duyet.md`) — tổng 742 test trên nhánh.
- 2026-07-21: **phase 4 hoàn thành sớm** (theo yêu cầu người dùng): hub thêm domain Đơn hàng + notify webhook (`5cc044c`) ⇄ client đẩy đơn sau sync qua cờ `hub_synced_at` (`5a0d27f`); **đã deploy hub lên VPS** (health ok, có backup). Fork `hub/` bên repo cũ chính thức bỏ.
- 2026-07-21: merge thêm 3 việc ngoài lộ trình: checkbox "Xóa profile và tạo lại" (`ffc346c`), **ribbon menu 4 tab + gộp 2 màn Cài đặt** (`ab8ceb3` — thay toàn bộ điều hướng shell, đã kiểm thị giác 4 tab), đang làm: sync shop BigSeller → Đơn hàng. Nhánh hiện 767 test.
